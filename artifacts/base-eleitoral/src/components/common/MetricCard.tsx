    ...
    <Card className={`premium-card premium-card-hover overflow-hidden border-t-4 ${classes.border}`}>
      <CardContent className="p-4 sm:p-5">
        ...
        {loading ? (
          ...
          <div className="text-2xl font-extrabold leading-none tracking-tight text-slate-950">{displayValue}</div>
        )}
        <div className="mt-1.5 text-xs font-bold uppercase tracking-[0.1em] text-slate-500">{label}</div>
        {helper ? <div className="mt-2 text-xs font-medium text-slate-400">{helper}</div> : null}
      </CardContent>
    </Card>
  );
}
